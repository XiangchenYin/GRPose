...
def init_model(sd_weights_path, config_path, output_path):
    ...
    for sk in scratch_dict.keys():
        if sk.replace('control_model.', 'model.diffusion_model.') in pretrained_weights.keys():
            target_dict[sk] = pretrained_weights[sk.replace('control_model.', 'model.diffusion_model.')].clone()
        else:
