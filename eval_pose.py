...
def safe_write(file_path, content, retries=3):
    for _ in range(retries):
        try:
            ...
            print(f"Failed to write to {file_path}: {e}. Retrying...")

safe_write('output.txt', 'Some content')
...
def main():
    ...
    parser.add_argument("--map_file", 
                        type=str, 
                        default=None,
                        help="multiGPU infer for splited map_file")
    ...
    parser.add_argument("--base_model", 
                        type=str, 
                        default=None, help='Base Diffusion Model. If None, it is SD1.5.') # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    if opt.base_model is not None:
        print('Change Base Model:', opt.base_model)
        # path = '/mnt/pfs-mc0p4k/tts/team/digital_avatar_group/sunwenzhang/xcyin/GraphPose-Model/civi-models/墨幽二次元_v2.safetensors'
        path = opt.base_model
    ...
    # define test dataset
    if opt.map_file is not None:
        
        config_dict = OmegaConf.to_container(config, resolve=True)  
        config_dict['data']['params']['test']['params']['map_file'] = opt.map_file
        config1 = OmegaConf.create(config_dict)
        dataset = instantiate_from_config(config1.data.params.test)
    else:
