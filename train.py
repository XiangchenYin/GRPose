...
def main():
    ...
    trainer_config["gpus"] = opt.devices
    ...
    m, u = model.load_state_dict(load_state_dict(opt.control_ckpt, location='cpu'), strict=True)
    if len(m) > 0:
        print("missing keys:")
        print(m)
    if len(u) > 0:
    ...
    # Train!
    # trainer = pl.Trainer(gpus=8, precision=32, plugins = LightningEnvironment(),
    #                       max_epochs=max_epochs, num_nodes=1, strategy='ddp')
