...
def predict(args):
    ...
    if args.controlnet:
        cv2.imwrite(os.path.join(args.save_path,'controlnet'+save_name), np.concatenate(controlnet_result,0))
    if args.t2i:
        cv2.imwrite(os.path.join(args.save_path,'t2iadapter'+save_name), np.concatenate(t2i_result,0))
...
def main(args):
    if args.controlnet:
        ...
        CONTROLNET_BASE_MODEL_CKPT='../GraphPose-Model/control_v11p_sd15_openpose.pth'
        CONTROLNET_MODEL_CKPT='models/v1-5-pruned.ckpt'
    ...
    if args.t2i:
        ...
        class T2I_OPT():
            def __init__(self) -> None:
                ...
                self.sd_ckpt="models/v1-5-pruned.ckpt"
                self.adapter_ckpt="../GraphPose-Model/t2iadapter_openpose_sd14v1.pth"
...
if __name__=="__main__":
    ...
    parser.add_argument(
        "--humansd_config",
        type=str,
        default="configs/tgpose/humanart_poseloss.yaml",
        help="the config file of HumanSD"
    )
    parser.add_argument(
        "--humansd_checkpoint",
        type=str,
        default="experiments/tgpose/tgpose_HumanArt-8-L40S-poseloss/epoch=000021.ckpt",
        help="the checkpoint of HumanSD"
    )
    parser.add_argument(
        "--mmpose_config",
        type=str,
        default="configs/mmpose/configs/body/2d_kpt_sview_rgb_img/associative_embedding/coco/higherhrnet_w48_coco_512x512_udp.py",
        help="the config file of human pose estimator"
    )
    parser.add_argument(
        "--mmpose_checkpoint",
        type=str,
        default="models/higherhrnet_w48_humanart_512x512_udp.pth",
