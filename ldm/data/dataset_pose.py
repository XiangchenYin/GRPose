...
class PoseDataset(Dataset):
    def __init__(self,
        ...
        self.train_transform = A.Compose([
                ...
                #A.VerticalFlip(p=0.1), # do we need this?
                # A.Rotate(p=0.1),
    ...
    def __getitem__(self, i):
        ...
        while True:
            try:
                ...
                # augmentation
                if self.phase == 'train':
                    transformed = self.train_transform(image=image, keypoints=pose_list, person_keypoint_index=self.person_keypoint_index)
                else:
                    transformed = self.eval_transform(image=image, keypoints=pose_list, person_keypoint_index=self.person_keypoint_index)
                ...
                # normalize to [-1,1]
                return_example={}
                return_example["img_path"] = example["img_path"]
                return_example["jpg"] = (scaled_image / 255 *2 - 1.0).astype(np.float32)
                
                return_example["pose"] = scaled_pose # (10, 17, 3)
                
                if self.phase == 'train':
                    return_example["txt"] = example['prompt'] if random.random() >= 0.5 else ""
                else:
                ...
                return_example["hint"]= (pose_img / 255 *2 - 1.0).astype(np.float32)
                return_example["pose_path"] = example["pose_path"]
